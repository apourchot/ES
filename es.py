...
def compute_ranks(x):
    ...
    Note: This is different from scipy.stats.rankdata, 
...
class OpenES:
    ...
    def ask(self, batch_size):
        ...
        if self.antithetic:
            ...
            self.epsilon = np.random.randn(batch_size, self.num_params)

        solutions = self.mu.reshape(
            1, self.num_params) + self.epsilon * self.sigma

        return solutions

    def tell(self, solutions, reward_table_result):
        # input must be a numpy float array
        assert(len(reward_table_result) ==
               self.popsize), "Inconsistent reward_table size reported."
    ...
    def get_distrib_params(self):
        return self.mu, 1 / self.sigma * np.eye(self.num_params)
...
class PEPG:
    ...
    def tell(self, reward_table_result):
        ...
        if (self.sigma_alpha > 0):
            ...
            reward_avg = (reward[:self.batch_size] +
                          reward[self.batch_size:]) / 2.0
            rS = reward_avg - b
            delta_sigma = (np.dot(rS, S)) / \
                (2 * self.batch_size * stdev_reward)
